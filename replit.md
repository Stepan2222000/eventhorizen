...
- **Global Scrolling Fixed**: Resolved system-wide scrolling issue where users couldn't scroll down to see all content and had to zoom out browser. **Root cause**: Main content container in `App.tsx` had `overflow-hidden` which blocked all vertical scrolling. **Solution**: Changed line 49 from `<main className="flex-1 overflow-hidden">` to `<main className="flex-1 overflow-y-auto">`. This one-line CSS fix restored vertical scrolling across all pages while keeping sidebar fixed. E2E tested on Dashboard (scrolls 217px to show bottom stats), Stock Details (both tables accessible), Article Search (all results visible), and Add Movement (form fully accessible). No layout regressions.
...
- **Independent Table Scrolling and Visual Improvements**: Resolved issue where user couldn't scroll down to see Sales History section when Purchase History table was very long, and improved visual consistency between tables. **Root cause**: shadcn Table component has built-in overflow-auto wrapper that prevented max-height constraints from working properly. **Solution Phase 1**: Replaced all shadcn Table components with plain HTML table/thead/tr/th/tbody/td elements in both tables. **Solution Phase 2**: Reduced table heights from 500px to 300px and improved purchase table styling to match sales table (thead: bg-muted/50, tbody cells: p-4 align-middle, rows: border-b). Each table now wrapped in `<div className="relative max-h-[300px] overflow-auto border rounded-md">` container. **Result**: Both sections are now simultaneously visible with minimal main page scrolling, independent scrolling within each 300px container, and consistent visual design across both tables. All inline editing functionality, test IDs preserved. **Cache issue**: Vite cache (node_modules/.vite) had to be cleared for Tailwind to recompile new classes. E2E tested: both tables scroll independently at 300px height, visual parity confirmed.
