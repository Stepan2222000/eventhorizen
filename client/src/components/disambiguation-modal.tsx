...
export function DisambiguationModal({ 
  ...
  // Safety check: ensure matches is a valid array
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[80vh]">
        <DialogHeader>
          <DialogTitle>Выберите SMART код</DialogTitle>
          <DialogDescription>
              ...
              <div className="flex items-start justify-between mb-1">
                ...
                {match.brand && (
                  <span className="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium bg-muted text-muted-foreground">
                    {Array.isArray(match.brand) ? match.brand.join(', ') : match.brand}
                  </span>
                )}
              </div>
              {match.articles && (
                <div className="text-xs text-muted-foreground mb-1">
                  ...
                  <span className="font-mono">{Array.isArray(match.articles) ? match.articles.join(', ') : match.articles}</span>
                </div>
              )}
              {match.description && (
                <div className="text-xs text-foreground mb-1">
                  <span className="font-semibold">Описание: </span>
                  {Array.isArray(match.description) ? match.description.join(', ') : match.description}
                </div>
              )}
